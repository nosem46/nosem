...
static inline int rect_equal(struct drm_rect left, struct drm_rect right)
{
	return (left.x1 == right.x1) && (left.x2 == right.x2) && (left.y1 == right.y1) && (left.y2 == right.y2); 
}

static inline int wh_equal(struct wh left, struct wh right)
{
	return (left.src_w == right.src_w) && (left.src_h == right.src_h) && (left.dst_w == right.dst_w) && (left.dst_h == right.dst_h); 
}
				      ...
				      struct drm_atomic_state *state)
{
	...
	static u64 modifier_old;
	static u32 format_old;
	static u32 flags_old;
	static struct wh wh_new, wh_old;
	static struct drm_rect src_old, dest_old;
	...
	if(priv->viu.osd1_reset || fb->modifier != modifier_old){
		priv->viu.osd1_upd |= UPDATE_CFG;

		if (fb->modifier & DRM_FORMAT_MOD_ARM_AFBC(MESON_MOD_AFBC_VALID_BITS))
	...
	if(priv->viu.osd1_reset || fb->format->format != format_old){
	...
	wh_new.src_w = fixed16_to_int(new_state->src_w);
	wh_new.src_h = fixed16_to_int(new_state->src_h);
	wh_new.dst_w = new_state->crtc_w;
	wh_new.dst_h = new_state->crtc_h;
	...
	if(priv->viu.osd1_reset || new_state->crtc->mode.flags != flags_old
	    || !wh_equal(wh_new, wh_old) || !rect_equal(dest, dest_old)){
		priv->viu.osd1_upd |= UPDATE_SCL;

		hf_phase_step = ((wh_new.src_w << 18) / wh_new.dst_w) << 6;
		vf_phase_step = (wh_new.src_h << 20) / wh_new.dst_h;
		...
		if (wh_new.src_h != wh_new.dst_h || wh_new.src_w != wh_new.dst_w) {
			priv->viu.osd_sc_i_wh_m1 = SCI_WH_M1_W(wh_new.src_w - 1) |
						   SCI_WH_M1_H(wh_new.src_h - 1);
	 ...
	 */
	if(priv->viu.osd1_reset || !wh_equal(wh_new, wh_old) || !rect_equal(new_state->src, src_old) || !rect_equal(dest, dest_old)){
		priv->viu.osd1_upd |= UPDATE_CFG;
		...
		priv->viu.osb_blend0_size = wh_new.dst_h << 16 | wh_new.dst_w;
		priv->viu.osb_blend1_size = wh_new.dst_h << 16 | wh_new.dst_w;
	}

	/* Update Canvas with buffer address */
	gem = drm_fb_dma_get_gem_obj(fb, 0);

	if(priv->viu.osd1_reset || priv->viu.osd1_addr != gem->dma_addr || priv->viu.osd1_stride != fb->pitches[0] 
		|| priv->viu.osd1_height != fb->height || priv->viu.osd1_width != fb->width){
        
		priv->viu.osd1_upd |= UPDATE_FB;
	...
	if (priv->viu.osd1_afbcd) {
		...
		priv->viu.osd1_blk2_cfg4 = meson_g12a_afbcd_line_stride(priv);
	}
	...
	priv->viu.osd1_reset = false;
	modifier_old = fb->modifier;
	format_old = fb->format->format;
	flags_old = new_state->crtc->mode.flags;
	wh_old = wh_new;
	src_old = new_state->src; dest_old = dest;
}
