					  ...
					  struct drm_atomic_state *state)
{
	...
	priv->viu.osd1_reset = true;
	priv->viu.osd2_reset = true;
	priv->viu.vd1_reset = true;
					   ...
					   struct drm_atomic_state *state)
{
	...
	priv->viu.osd1_upd = UPDATE_NONE;
	priv->viu.osd2_upd = UPDATE_NONE;
	priv->viu.vd1_upd = UPDATE_NONE;
...
static void nosem_osd1_commit(struct meson_drm *priv)
{
	...
	if (priv->viu.osd1_commit) {
        ...
        if(priv->viu.osd1_upd & UPDATE_CFG){
		...
		if(priv->viu.osd1_upd & UPDATE_SCL){
		...
		if (priv->viu.osd1_upd & UPDATE_FB && !priv->viu.osd1_afbcd)
...
static void nosem_osd2_commit(struct meson_drm *priv)
{
	struct meson_crtc *meson_crtc = to_meson_crtc(priv->crtc);
	unsigned long flags;
	
	/* Update the OSD registers */
	if (priv->viu.osd2_commit) {
		spin_lock_irqsave(&priv->reg_lock, flags);
		...
		writel_relaxed(priv->viu.osd_blend_din1_scope_h,
				priv->io_base + _REG(VIU_OSD_BLEND_DIN1_SCOPE_H));
		writel_relaxed(priv->viu.osd_blend_din1_scope_v,
				priv->io_base + _REG(VIU_OSD_BLEND_DIN1_SCOPE_V));

		meson_crtc->vsync_forced = false;

		if (priv->viu.osd2_upd & UPDATE_FB)
...
static void nosem_vd1_commit(struct meson_drm *priv)
{
	struct meson_crtc *meson_crtc = to_meson_crtc(priv->crtc);
	unsigned long flags;
...
static void nosem_g12a_crtc_enable_osd2(struct meson_drm *priv)
{
...
void meson_crtc_irq(struct meson_drm *priv)
{
	...
	nosem_osd2_commit(priv);
	nosem_osd1_commit(priv);
	nosem_vd1_commit(priv);
...
int meson_crtc_create(struct meson_drm *priv)
{
	...
	meson_crtc->enable_osd2 = nosem_g12a_crtc_enable_osd2;
