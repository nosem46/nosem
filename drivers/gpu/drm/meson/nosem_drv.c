...
#define DRIVER_DESC "NOSEM 5.15.181"
...
static irqreturn_t meson_irq(int irq, void *arg)
{
	...
	priv->t_vblank = ktime_get_ns();

	if(priv->t_vblank > priv->t_cursor + NOSEM_IDLE && priv->vsync_disabled)
		meson_venc_disable_vsync(priv);
	else
		drm_crtc_handle_vblank(priv->crtc);
	meson_crtc_irq(priv);
	writel_relaxed(0,priv->io_base + _REG(VENC_INTFLAG));
...
static int meson_drv_bind_master(struct device *dev, bool has_components)
{
	...
	if (!meson_vpu_is_compatible(priv, VPU_COMPATIBLE_G12A)) {
		dev_err(dev, "Only available to Meson G12A and later\n");
 		ret = -ENODEV;
		goto free_drm;
	}
...
free_canvas_osd2:
	meson_canvas_free(priv->canvas, priv->canvas_id_osd2);
free_canvas_vd1_2:
